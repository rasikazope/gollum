...
Node *
constrctTree(vector < int > iInOrderArray, vector < int > iPreOrderArray, int prestart, int preend, int instart, int inend) {
    if (instart > inend) {
        return NULL;
    }
    int rootval = iPreOrderArray[prestart];

    int rootindex = -1;

    for (int i = instart ; i <= inend; i++) {
        if (iInOrderArray[i] == rootval) {
            rootindex = i; 
        }
    }
    ...
    root->left = constrctTree(iInOrderArray, iPreOrderArray, prestart + 1, prestart + len,  instart, rootindex -1); 
    root->right = constrctTree(iInOrderArray, iPreOrderArray, prestart + len + 1, preend, rootindex + 1, inend); 
